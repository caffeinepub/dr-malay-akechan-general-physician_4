...
import { Upload, Trash2, Link, Image as ImageIcon, Save } from 'lucide-react';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
function getImageSrc(imageType: string, imageUrl: string, imageBase64: string): string | null {
...
export default function AboutEditor({ sessionToken, currentText, currentImageUrl, currentImageBase64 }: AboutEditorProps) {
  ...
  const [imageType, setImageType] = useState<'url' | 'base64'>(currentImageBase64 ? 'base64' : 'url');
  ...
  const saveBioMutation = useMutation({
    ...
    onError: (err: any) => {
      toast.error(`Failed to save biography: ${err?.message ?? 'Unknown error'}`);
    },
  });
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-2xl font-heading font-bold text-foreground mb-1">About Doctor</h2>
        <p className="text-muted-foreground text-sm">Edit the doctor's biography and profile photo.</p>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card p-6 space-y-4">
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium text-foreground" htmlFor="bio-text">
            Doctor's Bio
          </label>
          <textarea
            id="bio-text"
      ...
      <div className="rounded-xl border border-border bg-card p-6 space-y-5">
        ...
        <div className="flex items-start gap-6">
          ...
          <div className="flex-1 space-y-4">
            {/* Upload file */}
            <div className="space-y-2">
              <label className="text-sm font-medium text-foreground">Upload Image File</label>
              ...
              <p className="text-xs text-muted-foreground">Max 1.5MB. JPG, PNG, WebP supported.</p>
            </div>
        ...
        <div className="flex items-center gap-3 pt-2 border-t border-border">
          <button
            ...
            disabled={saveImageMutation.isPending || (!imageUrl && !imageBase64)}
            className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-cyan-500 hover:bg-cyan-400 text-white font-medium text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          {(imageUrl || imageBase64) && (
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <button className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-destructive/40 text-destructive hover:bg-destructive/10 font-medium text-sm transition-colors">
                  <Trash2 size={15} />
                  Remove Image
                </button>
              </AlertDialogTrigger>
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    This will permanently remove the doctor's profile image from the website. This action cannot be undone.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>

...
import { Plus, Pencil, Trash2, Save, X, Share2, Link } from 'lucide-react';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
    ...
    <div className="rounded-xl border border-cyan-500/30 bg-cyan-500/5 p-5 space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-foreground/80">Platform Name</label>
          <input
            type="text"
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-foreground/80">Profile URL</label>
          ...
          {value.url && !value.url.startsWith('http') && (
            <p className="text-xs text-amber-400">⚠ URL should start with https://</p>
          )}
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-heading font-bold text-foreground mb-1">Social Links</h2>
          <p className="text-muted-foreground text-sm">Manage social media profile links.</p>
        </div>
        <button
      ...
      {showAddForm && (
        <LinkForm
          ...
          onCancel={() => { setShowAddForm(false); setNewLink(emptyLink); }}
                ...
                <div className="rounded-xl border border-border bg-card p-4 flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="font-medium text-foreground text-sm">{link.platform}</p>
                      <p className="text-xs text-muted-foreground truncate">{link.url}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      onClick={() => { setEditingId(id); setEditLink({ ...link }); setShowAddForm(false); }}
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            Are you sure you want to delete the <strong>{link.platform}</strong> link? This action cannot be undone.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
